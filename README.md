# Storybook Addon Storybook Version Selector
This custom add-on for Storybook adds a version selector to the toolbar in the Storybook interface. The add-on automatically detects different versions of the library that are stored in subfolders within the `storybook-static` directory, generated by a script that creates a `versions.json` file.Us
